...
test("метапромт: размер рынка решается по SAM (TAM/SAM/SOM считаются)", () => {
  const p = buildPrompt(mp);
  assert.match(p, /Размер денег \(SAM, ₽\/год\)/, "фактор размера — на SAM");
  assert.match(p, /Решение по размеру принимаем по SAM/, "порог/решение по SAM");
  assert.match(p, /TAM/, "TAM считается");
  assert.match(p, /SOM/, "SOM считается как доп. ориентир");
});
