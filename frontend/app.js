...
function renderIntakeStep() {
  ...
  card.appendChild(
    el(
      "div",
      { class: "step-intro" },
      "Два пути. Можно пройти опросник по шагам (кнопка «Далее» внизу). Или вставить все ответы сразу: скопируй шаблон вопросов, заполни его (хоть с помощью Claude в чате), вставь обратно и нажми «Разобрать». Понимает и формат с метками [id], и обычный «Поле: ответ» (Название: …, Что делает: …) — теги (Ф/О/Д) в скобках в конце ответа подхватываются."
    )
  );
  ...
  parseBtn.addEventListener("click", () => {
    ...
    if (!n) {
      toast("Не нашёл ответов — нужен формат «Поле: ответ» или шаблон с [id]");
      return;
    }
    saveState();
